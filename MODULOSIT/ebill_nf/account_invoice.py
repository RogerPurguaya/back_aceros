		 	...
		 	for self_act in self:
				...
				for linea in self_act.invoice_line_ids:
					...
					for impuesto in linea.invoice_line_tax_ids:
						...
						if impuesto.ebill_tax_type in ['9']:
					...
					total_r    = float(decimal.Decimal(str(abs(unit_included*linea.quantity))).quantize(decimal.Decimal("1.11"), decimal.ROUND_HALF_DOWN))
					item={
