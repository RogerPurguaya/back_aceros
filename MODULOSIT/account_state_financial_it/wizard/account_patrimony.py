...
class account_patrimony_wizard(osv.TransientModel):
	...
	def do_rebuild(self):
		...
		t_i = [0,0,0,0,0,0]
		for j in self.env['account.move.line'].search([('move_id.date','>=',self.period_ini.date_start),('move_id.date','<=',self.period_fin.date_stop),('account_id.patrimony_type','!=',False)]):
			periodo = self.env['account.period'].search([('date_start','<=',j.move_id.date),('date_stop','>=',j.move_id.date),('special','=',j.move_id.fecha_special)])[0]
