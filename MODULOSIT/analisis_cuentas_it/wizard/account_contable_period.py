...
(
	select account_period.name as periodo,account_journal.name as diario,account_move.name as voucher,account_account_type_it.name as rubro,account_account.code as cuenta, account_move_line.debit as debe, account_move_line.credit AS haber,
...
left join account_account_type_it on account_account_type_it.id = account_account.type_it
left join account_period ap on ap.date_start >= account_move.fecha_contable and ap.date_stop <= account_move.fecha_contable
JOIN account_period ON ap.date_start <= account_move.date and ap.date_stop >= account_move.date  and ap.special = account_move.fecha_special
...
# left join account_period ap on ap.id = account_move.fecha_contable
# JOIN account_period ap ON ap.date_start <= am.date and ap.date_stop >= am.date  and ap.special = am.fecha_special
# left join account_journal on account_journal.id = account_move.journal_id
