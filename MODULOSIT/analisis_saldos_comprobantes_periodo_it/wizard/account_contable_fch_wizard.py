	...
	def do_rebuild(self):		
		...
		self.env.cr.execute("""  DROP VIEW IF EXISTS saldo_comprobante_periodo;
			...
			(
				...
				am.name as voucher
				from (
				select concat(account_move_line.partner_id,'-',account_id,'-',type_document_it,'-',nro_comprobante) as identifica,min(account_move_line.id),sum(debit)as debe,sum(credit) as haber, sum(debit)-sum(credit) as saldo, sum(amount_currency) as amount_currency from account_move_line
				...
				left join (
select concat(aml.partner_id,'-',aml.account_id,'-',aml.type_document_it,'-',aml.nro_comprobante)as ide, max(aml.date_maturity) as fin, am.id from
account_move am 
...
group by am.id,concat(aml.partner_id,'-',aml.account_id,'-',aml.type_document_it,'-',aml.nro_comprobante)
				...
				left join (select concat(partner_id,account_id,it_type_document,reference) as identifica,date,date_due from account_invoice) facturas on facturas.identifica=t.identifica
