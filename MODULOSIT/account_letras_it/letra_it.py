...
class account_letras_payment_factura(models.Model):
	...
	@api.depends('nro_comprobante','invoice_id','currency_id')
	def onchange_invoice_id(self):
		...
		cadfact = self.invoice_id.reference
		...
		vals = {
			...
			'mostrar':  'newwindow',
			'empresa': self.invoice_id.partner_id.id,
			'tipo' : 'A cobrar' if self.letra_payment_id.tipo == '1' else 'A pagar',
		...
		if self.invoice_id.reference:
			cadsql = "select * from saldo_comprobante_periodo where nro_comprobante ='"+self.invoice_id.reference+"'"
