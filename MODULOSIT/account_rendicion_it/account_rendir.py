...
class account_rendicion_it(models.Model):
	...
	def excel(self):
		if True:			
			...
			worksheet.write(4,4, u"Entrega",boldbord)
			worksheet.write(4,5, u"Rendición",boldbord)
			worksheet.write(4,6, "Saldo",boldbord)
			worksheet.write(4,7, "Glosa",boldbord)
			...
			if self.asiento1.id:
				...
				for w in self.asiento1.line_ids:
					if w.debit != 0:
						line = w

				saldo += line.debit - line.credit
				worksheet.write(x,0,line.move_id.date if line.move_id.date else '' ,bord )
				worksheet.write(x,1,line.nro_comprobante if line.nro_comprobante  else '',bord )
				worksheet.write(x,2,line.partner_id.nro_documento if line.partner_id.nro_documento  else '',bord)
				worksheet.write(x,3,line.partner_id.name if line.partner_id.name  else '',bord)
				worksheet.write(x,4,line.debit ,numberdos)
				worksheet.write(x,5,line.credit ,numberdos)
				worksheet.write(x,6,saldo ,numberdos)
				worksheet.write(x,7,line.name if line.name else '' ,bord)
				x = x +1


			for line in self.env['account.move.line'].search([('rendicion_id','=',self.id),('move_id.state','!=','draft'),('account_id','=',param.deliver_account_me.id if self.caja_entrega.currency_id.id else param.deliver_account_mn.id)]):
				saldo += line.debit - line.credit
				worksheet.write(x,0,line.move_id.date if line.move_id.date else '' ,bord )
				worksheet.write(x,1,line.nro_comprobante if line.nro_comprobante  else '',bord )
				worksheet.write(x,2,line.partner_id.nro_documento if line.partner_id.nro_documento  else '',bord)
	...
	@api.one
	def get_saldo(self):
		self.saldo =( ( (self.monto* self.tc_personalizado) if self.tc_personalizado else (self.monto* self.tc_oficial) ) if self.caja_entrega.currency_id.id else (self.monto) ) - self.monto_rendido
	...
	def get_monto_rendido(self):
		...
		if self.asiento1.id:
			asientos.append(self.asiento1.id)
		if self.asiento2.id:
			asientos.append(self.asiento2.id)

		self.env.cr.execute("""
				select credit-debit from account_move_line aml
				inner join account_move am on am.id = aml.move_id
				where aml.rendicion_id = """ + str(self.id) + """ and am.state != 'draft' and
				aml.account_id = """ +str(param.deliver_account_me.id if self.caja_entrega.currency_id.id else param.deliver_account_mn.id)+ """
				and am.id not in """ +str( tuple(asientos) )+ """
		 """)
		
		for i in self.env.cr.fetchall():
			tmp+= i[0]

		self.monto_rendido = tmp
	...
	def entrega(self):
		...
		linea1 = (0,0,{
			...
			'name':self.memoria,
			'currency_id':self.caja_entrega.currency_id.id,
			'amount_currency':-self.monto if self.caja_entrega.currency_id.id else 0,
			'debit':0,
			'credit':self.monto * tc if self.caja_entrega.currency_id.id else self.monto,
		...
		linea2 = (0,0,{
			...
			'currency_id':False,
			'amount_currency':0,
			'debit':self.monto * tc if self.caja_entrega.currency_id.id else self.monto,
			'credit':0,
			'date_maturity':self.fecha,
			'tc':tc,
		})
	...
	def rendido(self):
		...
		if not (param.loan_journal_me if self.caja_entrega.currency_id.id else param.loan_journal_mn).default_debit_account_id.id:
			raise osv.except_osv('Error!', u'Debe Configurarse las cuentas por defecto de los Diarios Rendicion.')			

		linea1 = (0,0,{
			'account_id': (param.loan_journal_me if self.caja_entrega.currency_id.id else param.loan_journal_mn).default_debit_account_id.id,
			'partner_id':self.empleado.id,
			'nro_comprobante':self.name,
			'name':self.memoria,
			'currency_id':self.caja_entrega.currency_id.id,
		...
		data = {
			'journal_id':param.loan_journal_me.id if self.caja_entrega.currency_id.id else param.loan_journal_mn.id,
			'date':self.fecha,
			'ref':self.name,
			'means_payment_it':self.medio_pago.id,
			'line_ids':lineas,
