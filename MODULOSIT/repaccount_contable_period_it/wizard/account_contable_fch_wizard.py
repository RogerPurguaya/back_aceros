...
from 
(
select ab.id,concat(ab.partner_id,'-',ab.account_id,'-',ab.type_document_it,'-',ab.nro_comprobante)as ide,ab.move_id,ab.partner_id,ab.account_id,ab.type_document_it,ab.nro_comprobante,ab.debit,ab.credit from account_move_line ab
...
select concat(xx.partner_id,'-',xx.account_id,'-',xx.type_document_it,'-',xx.nro_comprobante) as ide,sum(debit)-sum(credit) as saldo from account_move_line xx
...
group by concat(xx.partner_id,'-',xx.account_id,'-',xx.type_document_it,'-',xx.nro_comprobante)

) t1 on t1.ide=t0.ide
left join (
select am.id,concat(aml.partner_id,'-',aml.account_id,'-',aml.type_document_it,'-',aml.nro_comprobante)as ide, max(aml.date_maturity) as fin from
account_move am 
inner join account_move_line aml on aml.move_id = am.id 
group by am.id,concat(aml.partner_id,'-',aml.account_id,'-',aml.type_document_it,'-',aml.nro_comprobante)
